# ⚡ ArcXOS Operations Console // Hacked by Arunachalam M.

> **LEGAL / ETHICAL NOTICE**: This website and presentation suite are developed strictly for **EDUCATIONAL PURPOSES, ETHICAL HACKING DEMONSTRATIONS, and DEFENSIVE CYBERSECURITY RESEARCH**.

---

## 🚀 Features Overview

- **Cinematic Hacker Boot & Device Takeover Sequence (12–14 seconds)**:
  - Arch Linux UEFI initialization & kernel memory injection (`CVE-ARCXOS-2026-ROOTKIT`).
  - Dynamic hardware fingerprinting reading real client device parameters (OS, CPU threads, WebGL GPU renderer, resolution, network latency).
  - Escalation to `UID=0 (ROOT)` and screen glitch alarm takeover with sub-bass audio impact.
  - Realistic operational tone (**zero mention of simulation**).
  - `⚡ OVERRIDE / SKIP` button for live presentation pacing.
  - `☠️ REPLAY TAKEOVER` button to trigger the sequence on demand anytime.

- **Interactive Cyber Terminal (`arunachalam@arcxos-breach:~#`)**:
  - Full command execution with history (arrow keys), tab auto-complete, and touch buttons for mobile.
  - Commands: `whoami`, `arcxos`, `luna`, `target`, `scan`, `neofetch`, `exploit`, `matrix`, `google`, `replay`, `disclaimer`, `clear`.

- **Live Target Forensics & Telemetry Grid**:
  - Live extraction of visitor's hardware specifications, display viewport, battery, and network adapter.

- **Operator Dossier & Cyber Weapons Arsenal**:
  - Features Arunachalam's real-world contributions: **ArcXOS** (Arch Linux + BlackArch + Hyprland), **Luna AI** (`luna-llm` on AUR), **Netwatch IDS**, and **Cyberkit**.
  - Direct 1-click Google verification link: *"Search 'Arunachalam archlinux' on Google"*.

- **Custom Generated Hacker Visual Assets**:
  - High-resolution cyber images with lightbox inspection modal:
    - *The Master Infiltrator* (`assets/hacker_cyber_hoodie.jpg`)
    - *Ferocious Cyber Threat Entity with Arch Linux Crest* (`assets/ferocious_cyber_skull.jpg`)
    - *Shadow Network Operations* (`assets/anonymous_cyber_legion.jpg`)

- **Synthesized Web Audio API Engine**:
  - Browser-native synthesized audio: keystroke ticks, warning siren, sub-bass takeover drop, glitch static. Zero external mp3 dependencies!

- **Presentation Ready**:
  - One-click `📺 PRESENTATION MODE` (HTML5 Fullscreen API).
  - Fully responsive on mobile phones, tablets, laptops, and 4K projection screens.

---

## 💻 How to Launch Locally

Run a simple local server in this directory:

```bash
cd /home/arunachalam/arc
python3 -m http.server 8080
```

Then open your browser at:
```text
http://localhost:8080
```
